...
void Application::usage(int argc, char **argv){
	printf("Usage:\n");
	printf("    %s [-d] -f /path/to/app.conf [-s start|stop|restart]\n", argv[0]);
	printf("Options:\n");
...
void Application::parse_args(int argc,char *argv[])
{
    ...
    {0, 0, 0, 0}};
    while(-1!=(oc=getopt_long(argc,argv,":f:s:dhv",longopts,NULL)))
    {
        switch(oc)
        {
            case 'd':
                ...
                break;
            case 'f':
				app_args.conf_file = optarg;
                break;
			case 's':
				app_args.start_opt = optarg;
                break;
            case 'h':
                usage(argc, argv);
                exit(0);
	...
	if(app_args.conf_file.length() == 0 || app_args.start_opt.length() == 0 ||
		(app_args.start_opt != "start" && app_args.start_opt != "restart" && app_args.start_opt != "stop")){
		usage(argc, argv);
		exit(-1);
	}
}
...
void Application::init(){
	...
	{ // logger
		...
		log_output_dir = conf->get_str("logger.output_dir");
		if(log_output_dir == ""){
			log_output_dir = "./";
		}
...
void Application::kill_process(){
	...
	while(file_exists(app_args.pidfile)){
		fprintf(stderr, "file exists: %s, wait ...\n", app_args.pidfile.c_str());
		usleep(100 * 1000);
	}
	fprintf(stderr, "stop program succ!!!\n");
}
