...
#define APP_NAME "ai-server"
#define APP_VERSION AI_VERSION
...
void MyApplication::usage(int argc, char **argv){
	printf("Usage:\n");
	printf("    %s [-d] -f /path/to/ai.conf [-s start|stop|restart]\n", argv[0]);
	printf("Options:\n");
...
void MyApplication::run(){
	log_info("ai-server %s", APP_VERSION);
	...
	server = new(std::nothrow) AIServer(*conf, net);
	ASSERT_SYS(NULL != server, "try to new AIServer failed, no memory");
	
	log_info("pidfile: %s, pid: %d", app_args.pidfile.c_str(), (int)getpid());
	log_info("ai server started.");
	net->serve();
	SAFE_DELETE(net);
	SAFE_DELETE(server);
