...
rm -f src/version.h
echo "#ifndef AI_DEPS_H" >> src/version.h
echo "#ifndef AI_VERSION" >> src/version.h
echo "#define AI_VERSION \"`cat version`\"" >> src/version.h
...
case "$TARGET_OS" in
	...
	*)
		...
		echo "CLIBS += `pkg-config --libs protobuf`" >> build_config.mk
		echo "CFLAGS += -I \"$JEMALLOC_PATH/include\"" >> build_config.mk
		echo "CFLAGS += `pkg-config --cflags protobuf`" >> build_config.mk
	;;
esac
...
g++ -x c++ - -o $TMPDIR/ai_build_test.$$ 2>/dev/null <<EOF
