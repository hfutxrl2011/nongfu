...
class Logger{
	public:
		...
		static const int LEVEL_FRAME	= 6;
	...
	public:
		...
		int open(FILE *fp, int level=LEVEL_FRAME, bool is_threadsafe=false);
		int open(const char *filename, int level=LEVEL_FRAME,
			bool is_threadsafe=false, uint64_t rotate_size=0, const char *output_dir_ = "./");
		void close();
...
int log_open(FILE *fp, int level=Logger::LEVEL_FRAME, bool is_threadsafe=false);
int log_open(const char *filename, int level=Logger::LEVEL_FRAME,
	bool is_threadsafe=false, uint64_t rotate_size=0, const char *output_dir_ = "./");
int log_level();
...
#define log_frame(fmt, args...)	\
	log_write(Logger::LEVEL_FRAME, "%s(%d): " fmt, __FILE__, __LINE__, ##args)
