...
#define SSDB_E_OK 0
#define SSDB_E_SYS 10000
#define SSDB_E_PARAM 10001
#define SSDB_E_AUTH 10002

#define THROW_BREAK(fmt...) THROW(SSDB_E_OK, ##fmt)
#define THROW_SYS(fmt...) THROW(SSDB_E_SYS, ##fmt)
#define THROW_PARAM(fmt...) THROW(SSDB_E_PARAM, ##fmt)
#define THROW_AUTH(fmt...) THROW(SSDB_E_AUTH, ##fmt)
...
class ssdb_exception: public std::exception
{
	public:
		ssdb_exception()
		{
		...
		ssdb_exception(unsigned int line, const char *file, const char *function, const char *fmt, ...)
		{
			set_line(line);
			set_file(file);
...
#define FUNC_START \
	int __func_ret_code_by_wu_jing_baidu_inf_iis_not_same_as_other_variable__ = SSDB_E_SYS; \
try \
{ \
	do \
	{
#define FUNC_END(right_output, error_output) \
	...
	__func_ret_code_by_wu_jing_baidu_inf_iis_not_same_as_other_variable__ = SSDB_E_OK; \
	right_output("main logic was executed successfully [file: %s, function: %s].", __FILE__, __FUNCTION__); \
} \
catch(ssdb_exception &ex) \
{ \
